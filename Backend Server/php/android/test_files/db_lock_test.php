<?php 
	...
	if(isset($_GET['username']) && isset($_GET['password'])){
		...
		// connecting to db
		$db = new DB_CONNECT();
		$query = mysql_query("SELECT A.LOCKID,A.SOFT_LOCK_STATUS AS STATUS, A.COMMENTS FROM ACN_LOCK_INFO AS A, ACN_USER_INFO AS B WHERE (BINARY A.USERNAME='$username') AND (BINARY B.PASSWORD= '$password') AND A.USERNAME = B.USERNAME ORDER BY 1") 
		or die(mysql_error()); 
